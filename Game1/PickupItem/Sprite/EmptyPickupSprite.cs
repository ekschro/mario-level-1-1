...
namespace Game1
{
    public class EmptyPickupSprite : IPickupSprite
    {
        ...
        public void Draw()
        {
            ...
            int drawLocationX = (int)myGame.CurrentLevel.LevelCamera.PositionRelativeToCamera(emptyObject.GameObjectLocation().X);

            Rectangle sourceRectangle = new Rectangle(width * currentFrame, 0, width, TextureWareHouse.pickupTexture.Height);
            Rectangle destinationRectangle = new Rectangle(drawLocationX, (int)emptyObject.GameObjectLocation().Y, width, TextureWareHouse.pickupTexture.Height);
